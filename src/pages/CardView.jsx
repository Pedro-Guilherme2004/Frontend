...
import "../styles/cardview.css";   // <-- importa o CSS VERDE
...
const CardView = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (r.data && !r.data.error) { setDados(r.data); setError(null); }
        else { setDados(null); setError("Cartão não encontrado."); }
      } catch (err) {
        if (err.response?.status === 404) { localStorage.removeItem("card_id"); setError("Cartão não encontrado."); }
        else setError("Erro ao carregar o cartão.");
        setDados(null);
  ...
  useEffect(() => {
    ...
    if (token && dados?.emailContato) {
      ...
      } catch { setIsOwner(false); setEmailMismatch(false); }
    } else { setIsOwner(false); setEmailMismatch(false); }
  }, [dados]);

  const handleBackToEdit = () => {
    localStorage.setItem("card_temp_data", JSON.stringify({
      nome: dados?.nome || "", biografia: dados?.biografia || "", empresa: dados?.empresa || "",
      whatsapp: dados?.whatsapp || "", emailContato: dados?.emailContato || "", instagram: dados?.instagram || "",
      linkedin: dados?.linkedin || "", site: dados?.site || "", chave_pix: dados?.chave_pix || "",
    }));
  ...
  if (dados.foto_perfil) {
    if (dados.foto_perfil.startsWith("/uploads/")) fotoPerfilSrc = `${backendUrl}${dados.foto_perfil}`;
    else if (dados.foto_perfil.startsWith("data:image") || dados.foto_perfil.startsWith("http")) fotoPerfilSrc = dados.foto_perfil;
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      navigate("/");
    } catch { alert("Erro ao excluir o cartão."); }
  };
    ...
    <div className="card-page">
      <main className="card-main">
        <div className="border-type1">
          {emailMismatch && (
            <div style={{background:"#ffe6e6",border:"1px solid #d66767",borderRadius:8,padding:"16px 18px",color:"#822",marginBottom:18,textAlign:"center"}}>
              ...
              <button style={{background:"#c62828",color:"#fff",border:"none",borderRadius:6,padding:"8px 18px",fontWeight:600,marginTop:10,cursor:"pointer"}} onClick={handleBackToEdit}>
          ...
          {isOwner && (
            <>
              <button style={{ marginBottom:10, marginTop:10 }} onClick={() => navigate(`/card/edit/${dados.card_id}`)}>Editar Cartão</button>
              <button style={{ marginBottom:18, background:"#b30d0d", color:"#fff", borderRadius:8, border:"none", padding:"8px 20px", cursor:"pointer" }} onClick={handleDelete}>
                Excluir Cartão
              </button>
            </>
          )}
          ...
          {(dados.instagram || dados.linkedin || dados.site) && (
            <>
              ...
              <div className="button-group">
                {dados.instagram && <a href={dados.instagram} target="_blank" rel="noopener noreferrer"><button type="button"><img src="/insta.png" alt="Instagram" className="icon" /></button></a>}
                {dados.linkedin  && <a href={dados.linkedin}  target="_blank" rel="noopener noreferrer"><button type="button"><img src="/linkedin.png" alt="LinkedIn" className="icon" /></button></a>}
                {dados.site      && <a href={dados.site}      target="_blank" rel="noopener noreferrer"><button type="button"><img src="/website.png" alt="Site" className="icon" /></button></a>}
              </div>
            </>
          )}
